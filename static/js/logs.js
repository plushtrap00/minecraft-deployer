    ...
    .then(function(d) {
      ...
      if (c && (c.mods.length || c.caused_by.length)) {
        ...
        document.getElementById('crash-caused-by').innerHTML = c.caused_by
          .map(function(l) { return '<div>' + escHtml(l) + '</div>'; })
          .join('');
      }
    })
    .catch(function() {
...
function renderLog() {
  ...
  lines.forEach(function(line) {
    ...
    var show = true;
    if (filter === 'error') {
      show = low.indexOf('error') !== -1 || low.indexOf('fatal') !== -1;
    } else if (filter === 'warn') {
      show = low.indexOf('warn') !== -1;
    } else if (filter === 'errors-warns') {
      show = low.indexOf('error') !== -1 || low.indexOf('fatal') !== -1 || low.indexOf('warn') !== -1;
    } else if (filter === 'info') {
      show = low.indexOf('[info]') !== -1 || low.indexOf('/info]') !== -1;
    }
    if (!show) return;
